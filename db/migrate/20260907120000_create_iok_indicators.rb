...
class CreateIokIndicators < ActiveRecord::Migration[8.1]
  def change
    create_table :iok_indicators, id: :uuid, default: -> { "gen_random_uuid()" } do |t|
      ...
      # What a match means. Derived from the rule's level and tags on every
      # sync, so an admin correction lives in its own column rather than being
      # overwritten the next morning. See Iok::Severity.
